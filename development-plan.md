...
### Story 2.9: 全ビューからタスク詳細パネルを開く
**ゴール**: ユーザーがどのビューからでもタスクの詳細を編集できる

- [x] ガントビューでタスクバーをクリックして詳細パネルを開く
- [x] カンバンビューでタスクカードをクリックして詳細パネルを開く
- [x] ネットワークビューでノードをクリックして詳細パネルを開く
- [x] 詳細パネルのサイドパネル化（右側に固定表示）
- [x] パネルの開閉アニメーション

**デモ確認ポイント**:
- ガントビューでタスクバーをクリックすると詳細パネルが開く
- カンバンビューでカードをクリックすると詳細パネルが開く
- どのビューでも同じ詳細パネルで編集できる

---

### Story 2.10: サブタスク追加のインライン入力
**ゴール**: ユーザーがサブタスク追加時に直感的に入力できる

- [x] 親タスクの「+」ボタンクリックで入力フィールドを表示
- [x] 入力フィールドは親タスクの直下にインデント付きで表示
- [x] Enter で確定、Escape でキャンセル
- [x] 追加後、入力フィールドを維持して連続追加可能
- [x] フォーカスが外れたら自動的に入力フィールドを閉じる

**デモ確認ポイント**:
- 親タスクの下に入力フィールドが表示される
- サブタスクを連続して追加できる
- Escape でキャンセルできる

---

### Story 2.11: リストビューのドラッグ&ドロップ改善
**ゴール**: ユーザーがドラッグ操作の結果を事前に把握できる

- [ ] ドラッグ中にドロップ位置のインジケーターを表示
  - 上に挿入: タスクの上に青いライン
  - 下に挿入: タスクの下に青いライン
  - 子として追加: タスク全体をハイライト
- [ ] ドロップ位置によってインジケーターのスタイルを変更
- [ ] 移動後の階層構造をゴースト表示でプレビュー
- [ ] 無効なドロップ位置（循環参照になる場合）は赤色で警告表示
- [ ] ドラッグ中のカーソル変更

**デモ確認ポイント**:
- ドラッグ中に挿入位置が明確に見える
- 子として追加されるか、兄弟として追加されるかが一目でわかる
- 循環参照になる移動は視覚的に警告される

---

### Story 2.12: ガントチャートのリサイズハンドル改善
**ゴール**: ユーザーがタスクバーの端を直感的にドラッグできる

- [x] タスクバーホバー時にリサイズハンドルを表示
- [x] 左端ハンドルをドラッグして開始日を変更
- [x] 右端ハンドルをドラッグして終了日を変更
- [x] ドラッグ中にツールチップで変更後の日付を表示
- [x] カーソルをリサイズカーソルに変更
- [x] ドラッグ中のリアルタイムプレビュー

**デモ確認ポイント**:
- バーの端にマウスを乗せるとリサイズハンドルが表示される
- ドラッグ中に変更後の日付が見える
- 直感的に期間を調整できる

---

### Story 2.13: ネットワークビュー基本表示
**ゴール**: ユーザーがタスク間の依存関係をネットワーク図で確認できる

- [x] ネットワークビューのレイアウト構造
- [x] タスクをノード（カード）として表示
  - タスク名、ステータス、進捗率を表示
  - ステータスに応じた色分け
- [x] 依存関係をエッジ（矢印線）として表示
  - 依存タイプ（FS/SS/FF/SF）でスタイルを区別
  - ラグ/リードタイムをラベル表示
- [x] パン（ドラッグで視点移動）とズーム機能

**デモ確認ポイント**:
- ネットワークビューに切り替えてタスクがノードとして表示される
- 依存関係が矢印線で結ばれている
- ズームやパンで全体を俯瞰できる

---

### Story 2.14: ネットワークビューでのノード移動
**ゴール**: ユーザーがノードを自由に配置できる

- [x] ノードのドラッグ移動（基本的なカーソル変更）
- [ ] 移動位置の永続化（ビュー設定として保存）
- [ ] 複数ノードの同時選択と移動
- [x] 自動レイアウトボタン（依存関係に基づく配置）
- [ ] スナップグリッド（オプション）
...
### Story 2.15: ネットワークビューでの依存関係編集
**ゴール**: ユーザーがネットワークビューで依存関係を直接編集できる

- [x] ノード間をドラッグして新しい依存関係を作成
  - ドラッグ開始時に接続ポイントを表示
  - ドラッグ中に仮の線をプレビュー表示
- [x] エッジをクリックして選択
- [ ] 選択したエッジの依存タイプとラグを編集（ポップオーバー）
- [x] Delete キーまたはコンテキストメニューでエッジを削除
- [ ] 循環依存の検出と警告

**デモ確認ポイント**:
- ノード間をドラッグして依存関係を追加できる
- エッジをクリックして依存タイプを変更できる
- 依存関係を削除できる
